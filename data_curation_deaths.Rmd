...
url_ = 'https://en.wikipedia.org/wiki/Template:2019%E2%80%9320_coronavirus_pandemic_data/United_States_medical_cases'
webpage = read_html(url_)

data = as.data.table(rvest::html_table(webpage, fill= T)[1])
print(head(data,5))
str(data)
```

Notice that the data is formatted in a way where each column is made up of multiplicities of Regions such as North, Sourth, or Northdast. We want to re-format this data frame in such a way where each row represents a state and each column represents a distinct day
...
# extract regions and get rid of "." notation that they come in
regions = colnames(data)[1:56]
regions

split_regions = str_split(regions,'\\.')
regions = to_vec(for (i in split_regions) unlist(i[1]))


# make col names the state
states = unlist(data[1],use.names = F)
colnames(data) = states


# create hash objection with state as key and region as value
states_region <- setNames(as.list(regions[-1]), states[1:56][-1])
...
feb = c("Feb 9")
march = to_vec(for(i in 1:31) paste("Mar",i))
apr = to_vec(for (i in 1:13) paste("Apr", i))

# concat all dates
dates = c(feb, march,apr)

data = data[data$Date %in% dates,-(57:63)]

test = list(regions, key= states)
...
# melt per state
state_tbl = melt(data, id.vars = 'Date', variable.name = 'state')


state_tbl$region = unlist(states_region[state_tbl$state], use.names = F)
head(state_tbl)

# change value column => integer, fill all empty values with 1 and region => factor
state_tbl$value = nafill(as.integer(state_tbl[,value]), fill = 0)
state_tbl$region = as.factor(state_tbl[,region])

head(state_tbl)

# export file
# write.csv(state_tbl, 'reported_COVID_cases.csv')

# import another dataset in a "wide" format
data$region = regions
...
confirmed = fread('Covid-19.csv')

confirmed$Date = state_tbl$Date[1:45]



# inner join state_tbl with confirmed
test = merge(state_tbl, confirmed, by = 'Date')

test$V1 = NULL

# write.csv(test, 'COVID_reported_and_confirmed_cases.csv')

data = fread('COVID_reported_and_confirmed_cases.csv')
data$V1 = NULL
...
#date_ = "May 1"
  ...
  new_date = paste(new_month, day, sep = '/')
  return(new_date)
...
NOTE: 
1. The "Value" column is the only **unique** element. Each state's reported cases is independent of others

Therefore, if running pairwise tests, we would run it over the "value" column. 





Now that we have all our data nicely formatted, we can begin to look for associations between states, regions, and cases of COVID-19

Before we do that, lets introduce the function that will perform all our analysis
```{r}
